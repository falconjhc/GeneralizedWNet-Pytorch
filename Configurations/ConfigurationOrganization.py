...
from Utilities.file_operations import read_from_file, read_file_to_dict
...
class DatasetConfigObject(object):
    ...
    def __init__(self, trainAugmentContentTranslation, trainAugmentContentRotation, trainAugmentContentFlip, 
                 ...
                 inputStyleNum, inputContentNum, imgWidth, channels, label0VecTxt, label1VecTxt, dataRoot):
        ...
        self.inputContentNum=inputContentNum
        self.imgWidth=imgWidth
        self.channels=channels
        # self.loadedLabel0Vec=read_file_to_dict(os.path.join(dataRoot, label0VecTxt))
        # self.loadedLabel1Vec=read_file_to_dict(os.path.join(dataRoot, label1VecTxt))
...
class ParameterSetting(object):
    ...
    def ProcessNetworks(self, models):
        ...
        for thisPath in models:
            # tmp = thisPath[[ii.start() for ii in re.finditer("_",thisPath)][-1]+1:]
            thisName = thisPath.split('/')[-2]
            # this_device=ii[ii.find('@')+1:]
            # this_path=ii[:ii.find('@')]
            this_network_obj = NetworkConfigObject(name=thisName,
                                                   path=thisPath)
    ...
    def CheckGPUs(self, selectedDevice, availableGPU, availableCPU):
        ...
        # if selectedDevice in availableGPU:
        #     validDevice = selectedDevice
        # else:
        #     validDevice = availableCPU
    ...
    def GenerateExpID(self, wnet, encoder, mixer, decoder):
        if wnet == 'general':
            ...
            id = 'Exp-PlainWnet-%s' % (self.config.expID)
